#!/usr/bin/env python3
"""
지방 조직(Subcutaneous Fat) 데이터 생성 스크립트
...
def create_subcutaneous_fat_configs(num_samples=5):
    """지정된 개수의 지방 조직 시뮬레이션 설정 파일 생성"""
    
    # 기본 설정 파일 읽기
    with open("subcutaneous_fat_simulation.json", 'r') as f:
    ...
    for i in range(1, num_samples + 1):
        ...
        config["Session"]["RNGSeed"] = 0 + i  # 1, 2, 3, 4, 5
        config["Session"]["ID"] = f"subcutaneous_fat{i:03d}"
        
        # VolumeFile 경로 수정 (data/json에서 실행되므로 상대 경로 조정)
        config["Domain"]["VolumeFile"] = "../../cubic60.json"
        
        # 설정 파일 저장 (data/json 폴더에)
        config_file = f"data/json/subcutaneous_fat_simulation_{i}.json"
        with open(config_file, 'w') as f:
            json.dump(config, f, indent=4)
        
        configs.append(config_file)
        print(f"✅ 설정 파일 생성: {config_file} (RNGSeed: {0 + i})")
...
def run_simulations(configs):
    ...
    for i, config_file in enumerate(configs, 1):
        ...
        try:
            # MCXCL 실행 (상대 경로 사용)
            mcxcl_path = "../../../../bin/mcxcl"
            if not os.path.exists(mcxcl_path):
                # 절대 경로로 시도
                mcxcl_path = "/mnt/c/Users/user/Downloads/MCXCL/bin/mcxcl"
            
            # 절대 경로로 변환
            config_file_abs = os.path.abspath(config_file)
            cmd = f"{mcxcl_path} -f {config_file_abs} -g 0"
            print(f"명령어: {cmd}")
...
def move_result_files(sample_id):
    ...
    try:
        ...
        jnii_file = f"subcutaneous_fat{sample_id:03d}.jnii"
        if os.path.exists(jnii_file):
            os.rename(jnii_file, f"data/jnii/{jnii_file}")
            print(f"  📁 이동됨: {jnii_file} -> data/jnii/")
        else:
            # _vol이 있는 경우도 확인
            jnii_file_vol = f"subcutaneous_fat{sample_id:03d}_vol.jnii"
        ...
        # jdat 파일 이동
        jdat_file = f"subcutaneous_fat{sample_id:03d}_detp.jdat"
...
def visualize_subcutaneous_fat_samples():
    """지방 조직 샘플들을 시간적 동역학으로 시각화"""
    ...
    for i, jnii_file in enumerate(jnii_files, 1):
        ...
        try:
            ...
            # 샘플별 폴더 생성
            sample_folder = f'images/subcutaneous_fat{i:03d}'
            ...
            for t in range(time_gates):
                ...
                # 이미지 저장 (시간 게이트 포함)
                output_file = f'{sample_folder}/subcutaneous_fat{i:03d}_{t+1}.png'
...
def main():
    """메인 함수"""
    print("🩸 지방 조직(Subcutaneous Fat) 데이터 생성 및 시각화")
    print("=" * 50)
    ...
    # 현재 디렉토리 확인
    if not os.path.exists("subcutaneous_fat_simulation.json"):
        print("❌ 오류: subcutaneous_fat_simulation.json 파일을 찾을 수 없습니다.")
        print("subcutaneous_fat 디렉토리에서 실행해주세요.")
        return
    ...
    # 2. cubic60.json 파일 확인 (공유 폴더에서 참조)
    if not os.path.exists("../cubic60.json"):
        print("❌ 오류: cubic60.json 파일을 찾을 수 없습니다.")
        print("NEW 폴더에 cubic60.json이 있어야 합니다.")
        return
    
    # 3. 설정 파일 생성
    print(f"\n📝 설정 파일 생성 중... ({num_samples}개)")
    configs = create_subcutaneous_fat_configs(num_samples)
    ...
    # 5. 이미지 생성
    if success_count > 0:
        visualize_subcutaneous_fat_samples()
    ...
    if success_count > 0:
        ...
        for i in range(1, success_count + 1):
            print(f"  - subcutaneous_fat{i:03d}/")
