...
def create_whole_blood_configs(num_samples=5):
    """지정된 개수의 전혈 시뮬레이션 설정 파일 생성"""
    
    # 기본 설정 파일 읽기
    with open("whole_blood_simulation.json", 'r') as f:
...
def move_result_files(sample_id):
    ...
    try:
        ...
        # jdat 파일 이동
        jdat_file = f"whole_blood{sample_id:03d}_detp.jdat"
...
def visualize_whole_blood_samples():
    """전혈 샘플들을 시간적 동역학으로 시각화"""
    ...
    for i, jnii_file in enumerate(jnii_files, 1):
        ...
        try:
            ...
            # 샘플별 폴더 생성
            sample_folder = f'images/whole_blood{i:03d}'
            ...
            for t in range(time_gates):
                ...
                # 이미지 저장 (시간 게이트 포함)
                output_file = f'{sample_folder}/whole_blood{i:03d}_{t+1}.png'
...
def main():
    """메인 함수"""
    print("🩸 전혈(Whole Blood) 조직 데이터 생성 및 시각화")
    print("=" * 50)
    ...
    # 현재 디렉토리 확인
    if not os.path.exists("whole_blood_simulation.json"):
        print("❌ 오류: whole_blood_simulation.json 파일을 찾을 수 없습니다.")
        print("whole_blood 디렉토리에서 실행해주세요.")
        return
    ...
    configs = create_whole_blood_configs(num_samples)
