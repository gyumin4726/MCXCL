...
def create_air_configs(num_samples=5):
    """지정된 개수의 공기 시뮬레이션 설정 파일 생성"""
    
    # 기본 설정 (air_simulation.json 기준)
    base_config = {
        "Domain": {
            ...
            "Media": [
                ...
                {"mua": 0.000, "mus": 0.0, "g": 1.0, "n": 1.0}  # 공기의 광학적 특성
            ]
        },
        "Session": {
            "Photons": 100000,
            "RNGSeed": 12345,  # 이 값만 변경
            "ID": "air_test1"  # 이 값도 변경
        },
        "Forward": {
            "T0": 0.0e-09,
            "T1": 5.0e-09,
    ...
    for i in range(1, num_samples + 1):
        ...
        config["Session"]["ID"] = f"air{i:03d}"
        
        # 설정 파일 저장 (data/json 폴더에)
        config_file = f"data/json/air_simulation_{i}.json"
...
def move_result_files(sample_id):
    ...
    try:
        ...
        # jdat 파일 이동
        jdat_file = f"air{sample_id:03d}_detp.jdat"
...
def visualize_air_samples():
    """공기 샘플들을 시간적 동역학으로 시각화"""
    ...
    for i, jnii_file in enumerate(jnii_files, 1):
        ...
        try:
            ...
            # 샘플별 폴더 생성
            sample_folder = f'images/air{i:03d}'
            ...
            for t in range(time_gates):
                ...
                # 이미지 저장 (시간 게이트 포함)
                output_file = f'{sample_folder}/air{i:03d}_{t+1}.png'
...
def main():
    """메인 함수"""
    print("🌬️ 공기(Air) 매질 데이터 생성 및 시각화")
    print("=" * 50)
    ...
    # 현재 디렉토리 확인
    if not os.path.exists("air_simulation.json"):
        print("❌ 오류: air_simulation.json 파일을 찾을 수 없습니다.")
        print("air 디렉토리에서 실행해주세요.")
        return
    ...
    configs = create_air_configs(num_samples)
